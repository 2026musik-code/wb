 ...
 * Optimized, Secured, & Build-Fixed by AGEN ALENA NOVIANTI
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // API Proxying to hide API Key and prevent CORS issues
...
<head>
    ...
    <style>
        ...
        .overlay { position: absolute; bottom: 0; left: 0; right: 0; padding: 40px 20px; background: linear-gradient(transparent, rgba(0,0,0,0.9)); color: white; pointer-events: none; z-index: 5; }
        ...
        .icon-circle { width: 48px; height: 48px; background: rgba(255,255,255,0.1); backdrop-filter: blur(15px); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 22px; transition: 0.3s; }
    ...
    <script>
        ...
        function renderVideos(videos) {
            videos.forEach((video) => {
                ...
                // Properly escaping backticks and dollar signs for the build process
                card.innerHTML = \`
                    <video loop playsinline muted preload="auto" onclick="this.paused ? this.play() : this.pause()">
                        <source src="\\${videoSrc}" type="video/mp4">
                    </video>
                    <div class="sidebar">
                        <div class="action-btn"><div class="icon-circle"><i class="fas fa-heart"></i></div><span class="text-xs font-bold">Suka</span></div>
                        <div class="action-btn"><div class="icon-circle"><i class="fas fa-comment-dots"></i></div><span class="text-xs font-bold">Komen</span></div>
                        <div class="action-btn"><div class="icon-circle"><i class="fas fa-share"></i></div><span class="text-xs font-bold">Share</span></div>
                        <div class="mt-4"><img src="https://www.transparentpng.com/download/vinyl/classic-vinyl-record-transparent-background-5.png" class="music-disc" /></div>
                    </div>
                    <div class="overlay">
                        <h3 class="font-bold text-lg mb-1">@\\${author}</h3>
                        <p class="text-sm opacity-90 line-clamp-2 mb-4">\\${title}</p>
                        <div class="flex items-center gap-2"><i class="fas fa-music text-xs"></i><div class="overflow-hidden w-48 text-xs italic">Original Audio</div></div>
                    </div>\`;
